...
def setup_master(args, env=None, return_env=False):
    if env is None:
        env = make_multiagent_env(args.env_name, num_entities=args.num_entities, num_good_agents=args.num_good_agents,
    ...
    if args.env_name == 'simple_spread':
        ...
        num_entities = 2
    elif args.env_name == 'simple_tag1':
    ...
    pos_index = args.identity_size + 0
    for i, agent in enumerate(env.world.policy_agents):

        obs_dim = env.observation_space[i].shape[0]

        if hasattr(agent, 'adversary') and agent.adversary:
            # 此处同一个角色的智能体为同一个组，其神经网络是一样的
            if policy1 is None:
                ...
                policy1 = MPNN(args_value=args, input_size=pol_obs_dim, num_other_agents=num_friendly,
                               ...
                               pos_index=pos_index, mask_dist=args.mask_dist,entity_mp=entity_mp).to(args.device)#通信巨鹿为1 pos_index的作用
