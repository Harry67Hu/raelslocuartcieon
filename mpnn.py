...
class MPNN(nn.Module):
    def __init__(self, args_value, action_space, num_other_agents, num_agents, num_entities, role, input_size=16,
                 hidden_dim=128,
                 embed_dim=None, pos_index=0, norm_in=False, nonlin=nn.ReLU, n_heads=1, mask_dist=None,
                 entity_mp=False):
        super().__init__()
        ...
        # 这里是否需要根据角色不同手动设置input size，还是根据程序中返回的来看？
        self.encoder_pred = nn.Sequential(nn.Linear(self.input_size, self.h_dim),
                                     self.nonlin(inplace=True))

        self.encoder_prey = nn.Sequential(nn.Linear(self.input_size, self.h_dim),
                                         self.nonlin(inplace=True))

        self.messages_pred = MultiHeadAttention(n_heads=self.n_heads, input_dim=self.h_dim, embed_dim=self.embed_dim)
        self.messages_prey = MultiHeadAttention(n_heads=self.n_heads, input_dim=self.h_dim, embed_dim=self.embed_dim)

        self.update_pred = nn.Sequential(nn.Linear(self.h_dim + self.embed_dim, self.h_dim),
                                    self.nonlin(inplace=True))
        self.update_prey = nn.Sequential(nn.Linear(self.h_dim + self.embed_dim, self.h_dim),
                                         self.nonlin(inplace=True))
    ...
    def _fwd(self, inp):
        if self.env_name == "simple_spread":
            h = self._fwd_origin(inp)
        elif self.env_name == "simple_formation":
            h = self._fwd_origin(inp)
        elif self.env_name == "simple_tag":
            # print('simple_tag')
            h = self._fwd_tag3_3(inp, self.role)
            # print(self.role)
            # h = self._fwd_orgin_wang(inp, self.role)
        elif self.env_name == "simple_tag1":
            # print('yes')
            # h = self._fwd_origin(inp)
            h = self._fwd_two_transfer(inp, self.role)

        else:
            print('False')
        return h

    def _fwd_two_transfer(self, inp, role):
        agent_inp = inp[:, :self.input_size]
        mask = self.calculate_mask(agent_inp)

        if role == 1:
            h = self.encoder_pred(agent_inp)
            h = h.view(self.num_agents, -1, self.h_dim).transpose(0, 1)  # should be (batch_size/N,N,self.h_dim)
            for k in range(self.K):
                m, attn = self.messages_pred(h, mask=mask, return_attn=True)  # should be <batch_size/N,N,self.embed_dim>
                h = self.update_pred(torch.cat((h, m), 2))  # should be <batch_size/N,N,self.h_dim>
            h = h.transpose(0, 1).contiguous().view(-1, self.h_dim)
        else:
            h = self.encoder_prey(agent_inp)
            h = h.view(self.num_agents, -1, self.h_dim).transpose(0, 1)  # should be (batch_size/N,N,self.h_dim)
            for k in range(self.K):
                m, attn = self.messages_prey(h, mask=mask, return_attn=True)  # should be <batch_size/N,N,self.embed_dim>
                h = self.update_prey(torch.cat((h, m), 2))  # should be <batch_size/N,N,self.h_dim>
            h = h.transpose(0, 1).contiguous().view(-1, self.h_dim)
        return h


    def _fwd_origin(self, inp):
...
class RelationGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        # 1024个样本，就有1024个adj，那么就有1024个样本的8个agent之间的attention系数 那么就得和对应的1024个8个agent的样本输入相乘  attention格式应该为1024,8,8
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        zero_vec = -9e15 * torch.ones_like(e)
        # （condition, the value in the condition, 0）
        attention = torch.where(adj < 1, e,
                                zero_vec)  # 1024个样本，就有1024个adj，那么就有1024个样本的8个agent之间的attention系数 那么就得和对应的1024个8个agent的样本输入相乘  attention格式应该为1024,8,8
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, h=None, mask=None, return_attn=False):
        ...
        # print(torch.matmul(Q, K.transpose(2, 3)))
        # Calculate compatibility (n_heads, batch_size, n_query, graph_size)
        compatibility = (self.norm_factor / 1) * torch.matmul(Q, K.transpose(2, 3))
