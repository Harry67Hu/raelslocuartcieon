...
def train(args, return_early=False):
    ...
    for j in range(args.num_updates):
        ...
        if j%args.save_interval == 0 and not args.test:
            ...
            io.savemat(args.save_dir+'/reward'+str(j)+'.mat', {'array': REWARD})
        ...
        if args.eval_interval is not None and j%args.eval_interval==0:
            ob_rms = (None, None) if envs.ob_rms is None else (envs.ob_rms[0].mean, envs.ob_rms[0].var)
            print('===========================================================================================')
            _, eval_perstep_rewards, final_min_dists, num_success, eval_episode_len, collision_sum, step_sum = evaluate(args, None, master.all_policies,
            ...
            print('Collision sum {:d}| Step Sum {:d}'.format(collision_sum, step_sum))
            if final_min_dists and args.env_name != 'simple_tag'and args.env_name != 'simple_tag1':
                print('Final_dists_mean {}'.format(np.stack(final_min_dists).mean(0)))
                print('Final_dists_var {}'.format(np.stack(final_min_dists).var(0)))
            print(args.env_name, 'size:', args.arena_size, 'masking is ', args.masking)
            print(args.save_dir)
            print('===========================================================================================\n')
...
if __name__ == '__main__':
    ...
    pprint(vars(args))
    if not args.test:
        with open(os.path.join(args.save_dir, 'params.ijson'), 'w') as f:
            params = deepcopy(vars(args))
            params.pop('device') #移除device项目
            json.dump(params, f)
    train(args)
