...
#         elif args.env_name == 'simple_formation' or args.env_name == 'simple_line' or args.env_name == 'simple_tag' or args.env_name == 'simple_tag1':
...
#                     policies_list, ob_rms, args.render, render_attn=False)
...
def evaluate(args, seed, policies_list, ob_rms=None, render=False, env=None, master=None, render_attn=True):
    ...
    for t in range(num_eval_episodes):
        ...
        per_step_rewards[t] = episode_rewards / episode_steps
        num_success += info['n'][0]['is_success']
        collisions_sum += info['n'][0]['collisions']
        steps_sum += info['n'][0]['world_steps']
    ...
    return all_episode_rewards, per_step_rewards, final_min_dists, num_success, episode_length, collisions_sum, steps_sum
...
if __name__ == '__main__':
    ...
    policies_list = checkpoint['models']
    ob_rms = checkpoint['ob_rms']
    all_episode_rewards, per_step_rewards, final_min_dists, num_success, episode_length, collisions_sum, steps_sum = evaluate(
        args, args.seed,
        policies_list, ob_rms, args.render, render_attn=False)
    print("Average Per Step Reward {}\nNum Success {}/{} | Av. Episode Length {:.2f})"
          .format(per_step_rewards.mean(0), num_success, args.num_eval_episodes, episode_length))
    if final_min_dists:
        print("Final Min Dists {}".format(np.stack(final_min_dists).mean(0)))
    print("collisions, steps".format(collisions_sum, steps_sum))
    print(collisions_sum)
    print(steps_sum)
    print(collisions_sum / steps_sum)